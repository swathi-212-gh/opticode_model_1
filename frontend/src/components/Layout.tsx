...
import { Zap, LogOut, Terminal, UserCircle } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, user, onLogout }) => {
  ...
  // FIX: guard against empty string so avatar initial never renders "undefined"
        ...
        <nav className="flex-1 px-4 py-4 space-y-1">
          {navItems.map((item) => (
            ...
            >
              <item.icon
                className={`w-5 h-5 ${
                  location.pathname === item.path ? 'text-blue-400' : 'text-gray-500'
                }`}
              />
